...
def star_2(tet_mesh: SimplicialComplex) -> Float[t.Tensor, " tri"]:
    ...
    tet_tri_face_barycenters = Float[t.Tensor, "tet 4 3"] = t.mean(
        tet_vert_coords[:, [[j, k, l], [i, l, k], [i, j, l], [i, k, j]]], dim=-2
    )

    dual_edges = tet_barycenters - tet_tri_face_barycenters
...
def star_1(tet_mesh: SimplicialComplex) -> Float[t.Tensor, " edge"]:
    ...
    tet_edge_face_barycenters: Float[t.Tensor, "tet 6 3"] = t.mean(
    ...
    tet_tri_vec = tet_barycenters - tet_tri_face_barycenters
    tet_edge_vec = tet_barycenters - tet_edge_face_barycenters

    subareas: Float[t.Tensor, "tet 6"] = (
        t.abs(t.cross(tet_edge_vec, tet_tri_vec[:, [2, 1, 0, 0, 0, 1]], dim=-1))
        + t.abs(t.cross(tet_edge_vec, tet_tri_vec[:, [3, 3, 3, 2, 1, 2]], dim=-1))
    ) / 2.0
