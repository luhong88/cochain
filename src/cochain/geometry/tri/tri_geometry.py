...
def compute_tri_areas(
    vert_coords: Float[Tensor, "vert 3"], tris: Integer[LongTensor, "tri 3"]
) -> Float[Tensor, " tri"]:
    """
    ...
    area = 0.5 * torch.linalg.norm(torch.cross(edge_ij, edge_ik, dim=-1), dim=-1) + EPS
...
def compute_d_tri_areas_d_vert_coords(
    ...
    norm_s_len = torch.linalg.norm(norm_s, dim=-1, keepdim=True) + EPS
...
def cotan_weights(
    ...
    cot_s: Float[Tensor, "tri 3"] = edge_ns_ps_dot / (EPS + edge_ns_ps_cross)
