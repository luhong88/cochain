...
def _d2_tet_signed_vols_d2_vert_coords(
    ...
    vec: Float[Tensor, "tet 4 3"],
) -> Float[Tensor, "tet vert vert vert 3"]:
    """
    ...
    area_vec_grad: Float[Tensor, "tet 4 4 3"] = (
        tet_vert_coords[
            :,
    ...
    signed_vols_grad_grad = (
        torch.cross(
            area_vec_grad.view(-1, 4, 1, 4, 3), vec.view(-1, 1, 4, 1, 3), dim=-1
        )
        / 6.0
    )
...
def d_mass_1_d_vert_coords(
    ...
    d_signed_vols_d_vert_coords: Float[Tensor, "tet 4 3"] = (
        dompute_d_tet_signed_vols_d_vert_coords(vert_coords, tets)
    )

    tet_vol_vhp: Float[Tensor, "tet x=4 y=4 p=4 3"] = (
    ...
    bary_coords_grad_dot_grad: Float[Tensor, "tet x=4 y=4 p=4 3"] = (
    ...
    bary_coords_int: Float[Tensor, "tet x=4 y=4 1 1"] = torch.abs(
    ...
    bary_coords_int_grad: Float[Tensor, "tet x=4 y=4 p=4 3"] = (
    ...
    whitney_inner_prods_grad: Float[Tensor, "tet xy=6 rs=6 p=4 3"] = torch.zeros(
    ...
    whitney_inner_prods_grad_flat_signed: Float[Tensor, "tet 144"] = (
...
def d_mass_2_d_vert_coords(
    ...
    tet_signed_vols: Float[Tensor, "tet 1 1 1 1"] = compute_tet_signed_vols(
        vert_coords, tets
    ).view(-1, 1, 1, 1, 1)

    tet_vols = torch.abs(tet_signed_vols)

    d_signed_vols_d_vert_coords: Float[Tensor, "tet 1 1 4 3"] = (
        dompute_d_tet_signed_vols_d_vert_coords(vert_coords, tets)
    ).view(-1, 1, 1, 4, 3)
    ...
    sign_corrections_shaped: Float[Tensor, "tet 4 4 1 1"] = (
        sign_corrections.view(-1, 1, 4) * sign_corrections.view(-1, 4, 1)
    ).view(-1, 4, 4, 1, 1)
    whitney_inner_prods_shaped = whitney_inner_prods.view(-1, 4, 4, 1, 1)

    centroids: Float[Tensor, "tet 1 3"] = torch.mean(
        tet_vert_coords, dim=1, keepdim=True
    )
    ...
    whitney_inner_prod_grad: Float[Tensor, "tet i=4 j=4 p=4 3"] = (
    ...
    sum_delta = torch.einsum("pi,tjc->tijpc", identity, centroids - tet_vert_coords) / (
        9.0 * tet_vols
    )
...
def _d_cotan_weights_d_vert_coords(
    ...
    norm_tri_to_shaped: Float[Tensor, "tet 6 1 3"] = norm_tri_to.view(-1, 6, 1, 3)
    norm_tri_ho_shaped: Float[Tensor, "tet 6 1 3"] = norm_tri_ho.view(-1, 6, 1, 3)
    weight_o_shaped: Float[Tensor, "tet 6 1 1"] = weight_o.view(-1, 6, 1, 1)

    tet_signed_vols: Float[Tensor, " tet"] = compute_tet_signed_vols(vert_coords, tets)
    tet_signs = tet_signed_vols.sign()

    vols_shaped: Float[Tensor, "tet 1 1 1"] = torch.abs(tet_signed_vols).view(
        -1, 1, 1, 1
    )
    ...
    vol_grad: Float[Tensor, "tet 1 4 3"] = (
        dompute_d_tet_signed_vols_d_vert_coords(vert_coords, tets)
    ...
    norm_tri_to_grad: Float[Tensor, "tet 6 4 3"] = (
        tet_vert_coords[
            :,
            [
                [k, l, l, i],
    ...
    norm_tri_ho_grad: Float[Tensor, "tet 6 4 3"] = (
        tet_vert_coords[
            :,
            [
                [l, l, j, k],
    ...
    area_normal_dot_grad: Float[Tensor, "tet 6 4 3"] = torch.cross(
        norm_tri_ho_shaped, norm_tri_to_grad, dim=-1
    ) + torch.cross(norm_tri_to_shaped, norm_tri_ho_grad, dim=-1)

    # Compute the dense gradient of w_o wrt each vertex in a tet.
    weight_o_grad: Float[Tensor, "tet 6 4 3"] = (
...
def d_stiffness_d_vert_coords(
    ...
    tets: Integer[LongTensor, "tet 4"] = tri_mesh.tets
