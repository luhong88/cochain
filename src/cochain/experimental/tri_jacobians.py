...
def _d2_tri_areas_d2_vert_coords(
    ...
    vec: Float[Tensor, "tri 3 3"],
) -> Float[Tensor, "tri vert vert vert 3"]:
    """
    ...
    inner_prod: Float[Tensor, "tri 3 3 1 1"] = torch.sum(vec1 * vec2, dim=-1).view(
        -1, 3, 3, 1, 1
    ) * torch.eye(
        3,
        dtype=vert_coords.dtype,
    ...
    outer_prod: Float[Tensor, "tri 3 3 3 3"] = vec1.view(-1, 3, 3, 3, 1) * vec2.view(
        -1, 3, 3, 1, 3
    )
    outer_diff = -2.0 * outer_prod + outer_prod.transpose(-1, -2)

    triple_cross_grad: Float[Tensor, "tri 3 3 3 3"] = (
        inner_prod + outer_diff
    ) * sign_mask

    tri_area_grad: Float[Tensor, "tri 3 3"] = compute_d_tri_areas_d_vert_coords(
        vert_coords, tris
    )
    ...
    hess: Float[Tensor, "tri 3 3 3 3"] = -tri_area_grad.view(
        -1, 3, 1, 1, 3
    ) * tri_area_grad.view(-1, 1, 3, 1, 3) / tri_areas + triple_cross_grad / (
        4.0 * tri_areas
    )

    hvp = torch.einsum("txpcd,tyd->txypc", hess, vec)
...
def d_inv_star_2_d_vert_coords(
    ...
    dSdV_idx = torch.vstack(
        (
            torch.repeat_interleave(torch.arange(n_tris, device=tris.device), 3),
            tris.flatten(),
        )
    )
    dSdV_val = dAdV.flatten(end_dim=1)
    dSdV = torch.sparse_coo_tensor(dSdV_idx, dSdV_val, (n_tris, n_verts, 3)).coalesce()
...
def d_star_1_circumcentric_d_vert_coords(
    ...
    dWdV: Float[Tensor, "vert vert vert 3"] = _d_cotan_weights_d_vert_coords(
    ...
    canon_edges: Integer[Tensor, "edge 2"] = tri_mesh.edges
    ...
    all_edge_insert_loc = torch.searchsorted(canon_edge_idx_flat, all_edge_idx_flat)
    ...
    # edges.
    edge_insert_loc_clipped = torch.clip(all_edge_insert_loc, 0, n_edges - 1)
    ...
    dSdV = torch.sparse_coo_tensor(
        torch.vstack((dSdV_e_idx, dSdV_k_idx)),
        dSdV_val,
        (n_edges, n_verts, 3),
    ).coalesce()
...
def d_star_0_d_vert_coords(
    ...
    dAdV: Float[Tensor, "tri 3 3"] = compute_d_tri_areas_d_vert_coords(
        vert_coords, tris
    )
    ...
    dSdV_val = torch.repeat_interleave(dAdV, repeats=3, dim=0).flatten(end_dim=1) / 3.0
    dSdV = torch.sparse_coo_tensor(dSdV_idx, dSdV_val, (n_verts, n_verts, 3)).coalesce()
...
def d_mass_1_d_vert_coords(
    ...
    bary_coords_int: Float[Tensor, "tri x=3 y=3 1 1"] = torch.abs(tri_areas / 12.0) * (
        torch.ones((n_tris, 3, 3), dtype=dtype, device=device)
        + torch.eye(3, dtype=dtype, device=device).view(1, 3, 3)
    ).view(-1, 3, 3, 1, 1)

    bary_coords_int_grad: Float[Tensor, "tri x=3 y=3 p=3 3"] = (
    ...
    whitney_inner_prods_grad: Float[Tensor, "tri xy=3 rs=3 p=3 3"] = torch.zeros(
    ...
    whitney_inner_prods_grad_flat_signed: Float[Tensor, "tri 27"] = (
    ...
    dMdV = torch.sparse_coo_tensor(
        torch.vstack((dMdV_idx_xy, dMdV_idx_rs, dMdV_idx_p)),
...
def _d_cotan_weights_d_vert_coords(
    ...
    cot_grad_sn = torch.cross(unorm_s, uedge_ns, dim=-1) / (edge_ns_len * sin_squared_s)
    cot_grad_sp = torch.cross(unorm_s, -uedge_ps, dim=-1) / (
        edge_ps_len * sin_squared_s
    )
    ...
    cot_grad: Float[Tensor, "tri vert=3 neighbor=3 coord=3"] = torch.stack(
    ...
    asym_dWdV = torch.sparse_coo_tensor(
...
def d_stiffness_d_vert_coords(
    ...
    dSdV_diag: Float[Tensor, "vert vert 3"] = torch.sparse.sum(sym_dSdV, dim=1)
    # Note that the last dim is dense and does not show up in indices()
    diag_idx_i, diag_idx_k = dSdV_diag.indices()
    diag_idx = torch.vstack((diag_idx_i, diag_idx_i, diag_idx_k))

    # Generate the final, complete dSdV gradients.
    dSdV = torch.sparse_coo_tensor(
        torch.hstack((sym_dSdV.indices(), diag_idx)),
        torch.concatenate((sym_dSdV.values(), -dSdV_diag.values())),
