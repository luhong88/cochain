...
def test_sp_dense_mm_backward(A, device):
    ...
    loss_true.backward()
    A_grad_true = A_dense.grad.detach()[A_operator.sp_topo.idx_coo.unbind(0)]
    B_dense_grad_true = B_dense.grad.detach()

    B_dense.grad = None
    C_dense = A_operator @ B_dense
    loss = t.sum(C_dense**2)
    loss.backward()
    B_dense_grad = B_dense.grad.detach()
    A_grad = A_operator.val.grad.detach()
...
def test_sp_sp_mm_backward(A, device):
    ...
    loss_true.backward()
    A_grad_true = A_dense.grad.detach()[A_operator.sp_topo.idx_coo.unbind(0)]
    B_grad_true = B_dense.grad.detach()[B_operator.sp_topo.idx_coo.unbind(0)]

    C_operator = A_operator @ B_operator
    loss = t.sum(C_operator.val**2)
    loss.backward()
    B_grad = B_operator.val.grad.detach()
    A_grad = A_operator.val.grad.detach()
