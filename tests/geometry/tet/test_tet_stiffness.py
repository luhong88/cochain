...
def test_stiffness_kernel(two_tets_mesh: SimplicialComplex):
    """
    ...
    """
    bcc_S = stiffness_matrix(two_tets_mesh)
    row_sum = bcc_S.to_dense().sum(dim=-1)
    t.testing.assert_close(row_sum, t.zeros_like(row_sum))


def test_stiffness_symmetry(two_tets_mesh: SimplicialComplex):
    """
...
def test_stiffness_PSD(two_tets_mesh: SimplicialComplex):
    """
...
def test_stiffness_linear_precision(two_tets_mesh: SimplicialComplex):
    """
    The stiffness matrix acting on the interior of a 3D mesh vertex coordinates
    should result in zero.
    """
    bcc_S = stiffness_matrix(two_tets_mesh).to_dense()
    zero_tensor = bcc_S @ two_tets_mesh.vert_coords
    ...
    interior_mask = t.abs(two_tets_mesh.vert_coords).max(dim=-1).values < 1.0
