...
from cochain.geometry.tri_mesh import tri_hodge_stars, tri_laplacians
from cochain.geometry.tri_mesh.tri_stiffness import stiffness_matrix
...
def test_l0_direct_construction(two_tris_mesh: SimplicialComplex):
    ...
    """
    l0_via_cotan = tri_laplacians.laplacian_0(two_tris_mesh).to_dense()

    codiff_1 = tri_laplacians.codifferential_1(two_tris_mesh)
...
@pytest.mark.parametrize(
    "laplacian, betti",
    [
        (tri_laplacians.laplacian_0, 1),
        (tri_laplacians.laplacian_1, 0),
        (tri_laplacians.laplacian_2, 0),
    ],
)
...
def test_laplacian_0_kernel(tent_mesh: SimplicialComplex):
    l0 = tri_laplacians.laplacian_0(tent_mesh)
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (tri_laplacians.laplacian_1, tri_hodge_stars.star_1),
...
def test_codiff_1_adjoint_relation(tet_mesh: SimplicialComplex):
    ...
    s1 = t.diagflat(tri_hodge_stars.star_1(tet_mesh).to_dense())
